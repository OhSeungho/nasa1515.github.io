...
3. **vg(볼륨그룹) 생성** [형식 : vgcreate 볼륨그룹이름 pv나열]
...
4.  **lv(로지컬 볼륨) 생성** [형식 : lvcreate -n 논리볼륨이름 -l PE개수 볼륨그룹명]
...
6. **파일 시스템 생성 후(ext4) 수동마운트** (파운트포인트: /mnt/lvm1)
...
1.  **vg 사이즈 축소** [형식 : vgreduce vg이름 제거 pv지정]
...
----
  ...
  * **VG 확장** [방식 : 1. 디스크 추가장착, 2. 파티셔닝, 3.pv생성, 4.vg확장]
...
3. **VG 에 PV추가** [형식 : vgextend vg그룹 추가 할 PV]
...
* **(xfs) lee.lv 1G 확장하기** [형식 : lvextend -L 1G lv 절대경로]
