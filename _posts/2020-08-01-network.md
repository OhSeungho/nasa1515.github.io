...
## 1~2일차 간단 내용요약
   ...
   **1. OSI 7 계층 : Transport Layer 까지의 레이어정보, 기능, Divice, Protocol, PDU** 

         
**OSI 7 계층이란?**
: ISO(국제표준화기구)에서 개발한 모델 컴퓨터 네트워크 프로토콜로
...
<br/>
   ...
   **2. MAC Address/IP Address 표기방법(진수 표헌), 길이, 구조**
...
**용어 정리**
   ...
   - HUB - Flooding(플러딩)
	->  수신되는 링크를 제외한 나머지 모든 링크로 패킷을 단순하게 
복사 전송하는 일종의 무제어 포트 배정 
 ...
 *  TCP 프로토콜의 작동흐름
 ...
 이 메시지에 포함된 응답 번호는 (B + 1)
	...
	- **연결 종료** (Connection termination) -> (4-WAY)
		...
		수신자는 Acknowledgement Number 필드를 
		(Sequence Number + 1)로 지정 
